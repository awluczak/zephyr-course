...
ZTEST(ring_buf_push_pop, test_single_push_pop)
{
	...
	int ret;
	ret = rb_push(42);
	zassert_equal(ret, 0, "Push must result success (0)");
	ret = rb_pop(&v);
	zassert_equal(ret, 0, "Pop must result success (0)");
...
ZTEST(ring_buf_push_pop, test_fifo_order)
{
    ...
    zassert_true(rb_is_empty(), "Buffer must be empty after last pop");
}
...
ZTEST(ring_buf_push_pop, test_push_full_returns_enospc)
{
	...
	ret = rb_push(4);
	zassert_equal(ret, 0, "Push must result success (0)");
	zassert_true(rb_is_full(), "Buffer must be full after pushes");
	ret = rb_push(99);
	zassert_equal(ret, -ENOSPC, "Final push must result -ENOSPC");
...
ZTEST(ring_buf_boundaries, test_peek_does_not_consume)
{
	...
	zassert_equal(ret, 0, "Push must result success (0)");
	ret = rb_peek(&v);
	zassert_equal(ret, 0, "Peek must result success (0)");
...
ZTEST(ring_buf_boundaries, test_pop_null_returns_einval)
{
	...
	int ret;
	ret = rb_pop(NULL);
	zassert_equal(ret, -EINVAL, "Pop must result -EINVAL");
}
